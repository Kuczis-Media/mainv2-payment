...
function bucketSummary(cards, records) {
  const s = scheduler.stats(cards, records, 0), due = {};
  for (const q of cards) { const value = records[q.studyKey]; if (value?.dueAt) { const at = Date.parse(value.dueAt); due[at] = (due[at] || 0) + 1; } }
  return { ...s, due: Object.entries(due).map(([at, count]) => [Number(at), count]) };
}
function summary(index, now = Date.now()) {
  const value = { total: 0, new: 0, due: 0, failed: 0, hard: 0, attempts: 0, correct: 0, incorrect: 0 };
...
async function context(store, auth, repo, deck, create = true) {
  ...
  const resolved = progress.effectiveSettings(catalog), access = progress.sequenceAccessMap(catalog, progress.aggregateUser(active, catalog), active.preferences);
  const related = catalog.nodes.filter((n) => n.id === id || (n.type === 'quiz' && (n.settings.repositoryId || 'default') === repo && (n.settings.contentFile === deck || n.settings.quizId === deck)));
  if (!auth.roles.includes('admin') && related.some((n) => access[n.id]?.allowed === false)) failure('SEQUENCE_LOCKED', 409);
  ...
  return { definition, cards: scheduler.cards(definition.questions), generation, enabled, id, repo, related, catalog };
}
async function indexUpdate(store, userId, repo, deck, ctx, buckets) {
  ...
  const result = await storage.updateJson(store, indexKey(userId, repo, deck), {}, async (old) => {
    ...
    return { value: { version: 1, repositoryId: repo, deckId: deck, title: ctx.definition.metadata.title,
...
async function load(store, auth, repo, deck) {
  ...
  if (ctx.enabled && (old?.value?.generation !== ctx.generation || JSON.stringify(old?.value?.buckets) !== JSON.stringify(buckets) || old?.value?.title !== ctx.definition.metadata.title)) await indexUpdate(store, auth.userId, repo, deck, ctx, buckets);
