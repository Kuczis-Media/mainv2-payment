...
`chemdisk-exams` przechowuje odpowiedzi i wyniki niezależnie od przełącznika pasków postępu. Globalne `OFF` centralnego postępu zatrzymuje liczenie procentu kursu i ukrywa paski, ale nie może wyłączyć autosave, czasu ani zapisu samej próby, bo egzamin przestałby działać. Zaznaczenie odpowiedzi aktualizuje ekran natychmiast i trafia do bufora bieżącej karty w `sessionStorage`; serwer pozostaje źródłem prawdy. Zmienione odpowiedzi są wysyłane jedną paczką po około 8 sekundach albo dołączane do już potrzebnego żądania przejścia, zatwierdzenia lub zakończenia. Przejście jest optymistyczne w UI, lecz ograniczenia nadal są walidowane server-side. Dla 60 pytań wyświetlanych pojedynczo typowy pełny obieg bez dodatkowych obrazów, alertów i natychmiastowej informacji zwrotnej to około 63 wywołania Function (definicja, otwarcie, start, 59 przejść i submit), plus tylko te okresowe zapisy, przy których uczeń pozostawał na pytaniu dłużej niż interwał. Tryb natychmiastowy dodaje po jednym zatwierdzeniu na sprawdzone pytanie. Dokładny koszt zależy od konfiguracji, bieżącego planu Netlify i zachowania ucznia.
...
Centralny `netlify/ai-router.js` wybiera konfigurację modułu, pobiera sekret wyłącznie server-side i wywołuje adapter. Adaptery Gemini i OpenAI mają wspólne operacje wysyłania, testowania, pobierania modeli, normalizacji zużycia oraz błędów. Rozszerzenie o kolejnego dostawcę wymaga dodania adaptera, a nie zmiany kodu przeglądarki. Obecny chat korzysta z routingu `chat`. Klucze `GEMINI_API_KEY`/`OPENAI_API_KEY` są ścieżką migracyjną tylko wtedy, gdy magazyn panelu jest niedostępny albo nie zawiera żadnej konfiguracji. Niekompletna konfiguracja z panelu nie przełącza się cicho na inną konfigurację ani ENV.
...
Puste pole oznacza `unlimited`, a wszystkie aktywne reguły muszą przejść. Rezerwacja requestu oraz maksymalnego budżetu tokenów używa silnego odczytu Netlify Blobs i warunkowego zapisu ETag/CAS. Dzięki temu równoległe żądania nie omijają limitu. Providerowe `429`, awaria, nieprawidłowy klucz oraz kontrolowane limity ChemDisk mają różne kody. Normalne przekroczenie limitu zwraca `429`, a `AI_DISABLED_FOR_USER` — `403`, nie `500`.
