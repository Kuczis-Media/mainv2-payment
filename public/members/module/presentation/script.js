(async () => {
  ...
  const presentationId = String(params.get('presentation') || '').trim().toLowerCase();
  const repositoryId = String(params.get('repo') || 'default').trim().toLowerCase() || 'default';
  const preview = params.get('preview') === '1';
  const elements = {
    main: document.getElementById('presentation-player-main'), loading: document.getElementById('presentation-player-loading'),
    error: document.getElementById('presentation-player-error'), errorCopy: document.getElementById('presentation-player-error-copy'),
    player: document.getElementById('presentation-player'), title: document.getElementById('presentation-player-title'),
    outline: document.getElementById('presentation-player-outline'), outlineToggle: document.getElementById('presentation-player-outline-toggle'),
    stage: document.getElementById('presentation-player-stage'), previous: document.getElementById('presentation-player-previous'),
    next: document.getElementById('presentation-player-next'), position: document.getElementById('presentation-player-position'),
    progress: document.getElementById('presentation-player-progress'), fullscreen: document.getElementById('presentation-player-fullscreen'),
    theme: document.getElementById('presentation-player-theme'), save: document.getElementById('presentation-player-save')
  };
  const state = { definition: null, index: 0, visited: new Set(), urls: new Set(), materialId: '' };
  const progressApi = window.ChemProgress;

  function showError(message) { elements.loading.hidden = true; elements.player.hidden = true; elements.error.hidden = false; elements.errorCopy.textContent = message; }
  function fontStack(font) { return ({ roboto: 'Roboto, Arial, sans-serif', 'open-sans': '"Open Sans", Arial, sans-serif', montserrat: 'Montserrat, Arial, sans-serif', poppins: 'Poppins, Arial, sans-serif', lato: 'Lato, Arial, sans-serif', nunito: 'Nunito, Arial, sans-serif', lora: 'Lora, Georgia, serif', merriweather: 'Merriweather, Georgia, serif', playfair: '"Playfair Display", Georgia, serif', georgia: 'Georgia, serif', times: '"Times New Roman", serif', 'jetbrains-mono': '"JetBrains Mono", ui-monospace, monospace', 'source-code-pro': '"Source Code Pro", ui-monospace, monospace', mono: 'ui-monospace, monospace', arial: 'Arial, sans-serif', verdana: 'Verdana, sans-serif' })[font] || 'Inter, system-ui, sans-serif'; }
  async function requestDefinition() {
    if (!/^[a-z0-9][a-z0-9-]{0,79}$/.test(presentationId) || !/^[a-z0-9][a-z0-9-]{0,39}$/.test(repositoryId)) throw new Error('Nieprawidłowa referencja prezentacji.');
    const token = await window.ChemAuth.getAccessToken();
    const url = new URL('/.netlify/functions/presentation', location.origin);
    url.searchParams.set('presentation', presentationId); url.searchParams.set('repo', repositoryId); if (preview) url.searchParams.set('preview', '1');
    ...
    if (!response.ok) throw new Error(payload.error === 'PRESENTATION_NOT_PUBLISHED' ? 'Prezentacja nie została jeszcze opublikowana.' : payload.error || 'Nie udało się pobrać prezentacji.');
  ...
  function renderElement(item) {
    ...
    else if (item.type === 'button') { const link = document.createElement('a'); link.className = 'presentation-player-button'; link.textContent = item.label; link.href = item.href; if (/^https:\/\//.test(item.href)) { link.target = '_blank'; link.rel = 'noopener noreferrer'; } Object.assign(link.style, { color: item.color, background: item.background, borderRadius: `${item.borderRadius}px` }); node.append(link); }
    else if (item.type === 'code') { const code = document.createElement('pre'); code.className = 'presentation-player-code'; code.textContent = item.code; Object.assign(code.style, { color: item.color, background: item.background, fontSize: `${item.fontSize}px` }); node.append(code); }
    else if (item.type === 'embed') { const frame = document.createElement('iframe'); frame.className = 'presentation-player-embed'; frame.src = item.url; frame.title = item.title; frame.loading = 'lazy'; frame.referrerPolicy = 'strict-origin-when-cross-origin'; frame.setAttribute('sandbox', 'allow-scripts allow-same-origin allow-forms allow-presentation'); frame.setAttribute('allow', 'fullscreen; encrypted-media'); node.append(frame); }
    return node;
  }
  async function mediaBlob(reference, ownerRepository) { const shared = reference.startsWith('assets/shared/'); return window.ChemContentLibrary.readMediaBlob({ scope: shared ? 'shared' : 'local', materialKind: shared ? '' : 'presentation', materialId: shared ? '' : presentationId, reference, repositoryId: ownerRepository || repositoryId }); }
  async function loadImage(node, item) { try { const blob = await mediaBlob(item.ref, item.repositoryId); if (!node.isConnected) return; const url = URL.createObjectURL(blob); state.urls.add(url); const image = document.createElement('img'); image.src = url; image.alt = item.alt; image.style.objectFit = item.fit; image.style.objectPosition = `${item.focalX}% ${item.focalY}%`; image.style.borderRadius = `${item.borderRadius}px`; image.style.opacity = String(item.opacity ?? 1); node.replaceChildren(image); } catch (_) { node.textContent = 'Brak obrazu'; } }
  ...
  elements.next.addEventListener('click', () => { if (state.index < state.definition.slides.length - 1) { state.index += 1; render(); } else location.href = window.ChemModuleReturn?.url || '/members/'; });
