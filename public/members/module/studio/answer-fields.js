  ...
  function exam(question, onChange) {
    ...
    function renderChoices() {
      ...
      question.options.forEach((option, index) => {
        const row = make('div', 'answer-choice-row');
        row.classList.toggle('is-correct', question.correctAnswerIds.includes(option.answerId));
        const correct = make('input');
        correct.type = multiple ? 'checkbox' : 'radio';
        correct.name = `answer-${question.questionId}`;
        correct.checked = question.correctAnswerIds.includes(option.answerId);
        correct.setAttribute('aria-label', `Poprawna odpowiedź ${index + 1}`);
        correct.addEventListener('change', () => {
          question.correctAnswerIds = multiple
            ? question.options.filter((entry) => entry.answerId === option.answerId ? correct.checked : question.correctAnswerIds.includes(entry.answerId)).map((entry) => entry.answerId)
            : [option.answerId];
          list.querySelectorAll('.answer-choice-row').forEach((item, itemIndex) => {
            item.classList.toggle('is-correct', question.correctAnswerIds.includes(question.options[itemIndex].answerId));
          });
          notify();
        });
        row.append(correct, textField(`Odpowiedź ${String.fromCharCode(65 + index)}`, option.text, (value) => { option.text = value; notify(); }, true));
        row.append(action('Usuń', () => {
