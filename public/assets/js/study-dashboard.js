(function (root) {
  ...
  function render() {
    ...
    const totals = decks.reduce((s, d) => ({ due: s.due + d.due, new: s.new + d.new, hard: s.hard + d.hard }), { due: 0, new: 0, hard: 0 });
    status.textContent = decks.length ? `${totals.due} do powtórzenia · ${totals.new} nowych · ${totals.hard} trudnych (we wczytanych pulach).` : 'Nie masz jeszcze rozpoczętej puli.';
    decks.forEach((d) => {
